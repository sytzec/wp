...
?><article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		...
		<?php if ( is_single() || is_page() ) : ?>
			...
			<?php the_content(__('<span class="more-link">Lees meer</span>', 'simple')); ?>
		...
		<?php else : ?>	
			...
			<?php if (of_get_option('content_excerpt') == 'excerpt') : ?>
				...
				<p class="more-tag"><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'simple' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php _e('Lees meer', 'simple'); ?></a></p>
				
			<?php else : ?>
			
				<?php if ( ! has_excerpt() ) : ?>
				
				     <?php the_content(__('<span class="more-tag">Lees meer</span>', 'simple')); ?>
